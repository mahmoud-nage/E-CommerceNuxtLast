      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <div class="row mb-2">
              <div class="col-sm-6">
                <nuxt-link to="/ecommerce/flashdeals/create" class="btn btn-danger mb-2"><i class="mdi mdi-plus-circle mr-1"></i> {{$t('Add FlashDeal')}}</nuxt-link>
              </div>
              <div class="col-sm-6">
            ...
            <div class="table-responsive mb-0">
              <b-table table-class="table table-centered w-100" thead-tr-class="bg-light" :items="productData" :fields="fields" responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc"
                ...
                <template  v-slot:cell(countProduct)="data" >
                                <span v-if="data.item.countProduct >= 0"  class="badge" :class="{
                      'badge-success': data.item.countProduct <= 10,
                      'badge-warning': data.item.countProduct <= 20,
                      'badge-danger': data.item.countProduct > 20,
                    }">{{ data.item.countProduct }} {{ $t('product') }}</span>
                </template>

                <template v-slot:cell(action)>
                  <ul class="list-inline table-action m-0">
                    <li class="list-inline-item">
                      <NuxtLink to="/ecommerce/flashdeals/1/edit" class="action-icon"><i class="mdi mdi-square-edit-outline"></i></NuxtLink>
                    </li>
                    <li class="list-inline-item">
