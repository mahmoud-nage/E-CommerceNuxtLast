      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <div class="row mb-2">
              <div class="col-sm-6">
                <nuxt-link to="/ecommerce/subCategories/create" class="btn btn-danger mb-2"><i class="mdi mdi-plus-circle mr-1"></i> {{$t('Add Sub Category')}}</nuxt-link>
              </div>
              <div class="col-sm-6">
            ...
            <div class="table-responsive mb-0">
              <b-table table-class="table table-centered w-100" thead-tr-class="bg-light" :items="productData" :fields="fields" responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc"
                ...
                <template v-slot:cell(Category)="data">
                  ...
                  <h5 class="m-0 d-inline-block align-middle">
                    <a href="#" class="text-dark">{{ data.item.categoryNamee }}</a>
                  </h5>
                </template>
                ...
                <template  v-slot:cell(countSubSubCategory)="data" >
                                <span v-if="data.item.countSubSubCategory >= 0" class="badge" :class="{
                      'badge-success': data.item.countSubSubCategory <= 100,
                      'badge-danger': data.item.countSubSubCategory <= 0 ,
                      'badge-warning':
                        data.item.countSubSubCategory >= 150
                    }">{{ data.item.countSubSubCategory }} {{$t('SubSubCategory')}}</span>
                </template>
